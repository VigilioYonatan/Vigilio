...
function applyHttpMethod({
	method,
	path,
	options,
}: { method: HttpMethod; path: string; options?: HandlerOptions }) {
	if (process.env.NODE_ENV === "development" && path !== "/") {
		loadPackage("path-to-regexp", {
			context:
				"@" + method.charAt(0).toUpperCase() + method.slice(1).toLowerCase(),
			docsUrl:
				"https://next-api-decorators.vercel.app/docs/routing/route-matching",
		});
	}
	...
	return (
		...
		const methods: Array<HandlerMethod> =
			Reflect.getMetadata(HTTP_METHOD_TOKEN, target.constructor) ?? [];
...
function getOptions(
	pathOrOptions?: string | HandlerOptions,
	options?: HandlerOptions,
) {
