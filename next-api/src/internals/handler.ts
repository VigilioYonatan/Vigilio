...
async function runMiddlewares(
    ...
    for (const middleware of middlewares) {
        ...
        await new Promise<void>((resolve, reject) => {
            ...
            const fnResult = (middleware as NextMiddleware).call(
                this,
                req,
                res,
                (err) => {
...
async function runMainLayer(
    ...
    const classHeaders: Map<string, string> | undefined = Reflect.getMetadata(
        HEADER_TOKEN,
        target.constructor
    );
    ...
    const parameters = await Promise.all(
        parameterDecorators.map(async ({ location, name, index, fn }) => {
            if (location === "custom") {
    ...
    if (returnValue instanceof Stream) {
        ...
        res.setHeader(
            "Content-Disposition",
            `attachment; filename="${returnValue.filename}"`
        );
...
export function applyHandler(
    ...
    descriptor.value = async function (
        ...
        const classMiddlewares: Middleware[] | undefined = Reflect.getMetadata(
            MIDDLEWARE_TOKEN,
            target.constructor
        );
