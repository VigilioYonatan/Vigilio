...
function routeHandler(
    ...
    return (req: Request, res: Response, next: NextFunction) => {
        (req as any).information = (req as any).information ?? null;
        (req as any).user = (req as any).user ?? null;
        (req as any).t = (req as any).t ?? null;
    ...
    for (const { name, index, type } of params) {
        switch (type) {
            ...
            case ParameterType.INFORMATION:
                args[index] = getParam(req, "information", name) ?? null;
                break;
            case ParameterType.USER:
                args[index] = getParam(req, "user", name) ?? null;
                break;
            case ParameterType.T:
                args[index] = getParam(req, "t", name) ?? null;
                break;
        }
    }
