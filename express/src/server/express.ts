...
export async function attachControllers(
    ...
    const promises = controllers.map((controller: Type) =>
        registerController(app, controller, getController as any)
    );
...
export async function attachControllerInstances(
    ...
    const promises = controllers.map((controller: any) =>
...
function routeHandler(
    ...
    return (req: Request, res: Response, next: NextFunction) => {
        ...
        const result = (controller as any)[methodName].call(
            controller,
            ...args
        );
    ...
    for (const { name, index, type } of params) {
        switch (type) {
            ...
            case ParameterType.COOKIES:
                ...
                break;
            case ParameterType.INFORMATION:
                args[index] = getParam(req, "information", name);
                break;
            case ParameterType.USER:
                args[index] = getParam(req, "user", name!);
                break;
        }
    }
 ...
 */
function getParam(source: any, paramType: string | null, name: string): any {
