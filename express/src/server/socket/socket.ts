 ...
 */
export function attachSocketControllers(
    io: SocketIO.Server,
    controllers: Type[]
) {
    io.use((socket, next) =>
        middlewareSocketHandler(IO_MIDDLEWARE)(io, socket, next)
    );
 ...
 */
function attachController(io: SocketIO.Server, controller: Type) {
...
function socketMiddlewareHandler(
    ...
    return (packet, next) => {
         ...
         */
        const listener = meta.listeners.find(
            (lst: Listener) => lst.event === event
        );
...
function mapArguments(
    ...
    return params
        ...
        .map((param: Param) => {
            switch (param.type) {
                case ParameterType.IO:
                    return getWrapper(param, io);
                case ParameterType.Socket:
                    return getWrapper(param, socket);
                case ParameterType.Args:
                    return getArgs(args);
                case ParameterType.Ack:
                    return getAck(args);
            }
        });
}
