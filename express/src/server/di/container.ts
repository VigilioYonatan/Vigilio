...
export class Container {
    ...
    public static provide(providers: Provider[]) {
        providers
            ...
            .forEach((_provider: ClassProvider) =>
                this.registerClassProvider(_provider)
            );

        providers
            .filter((_provider: FactoryProvider) => _provider.useFactory)
            .forEach((_provider: FactoryProvider) =>
                this.registerFactoryProvider(_provider)
            );

        providers
            .filter((_provider: ValueProvider) => _provider.useValue)
            .forEach((_provider: ValueProvider) =>
                this.registerValueProvider(_provider)
            );
    }
    ...
    private static resolveProvider(
        ...
        const deps = provider.deps.map((dep: Dependency) => {
            const requesterProvider: StoreProvider = _requesters.find(
                (requester: StoreProvider) => requester.id === dep.id
            );

            if (requesterProvider) {
                throw new RecursiveProviderError(
                    _requesters,
                    requesterProvider
                );
            }
    ...
    private static registerClassProvider(provider: ClassProvider): void {
        ...
        const classProvider: StoreProvider = Store.findProvider(
            provider.useClass
        );
        const deps: Dependency[] = classProvider
            ? classProvider.deps
            : (provider.deps || []).map((dep: Injectable) => ({
    ...
    private static registerFactoryProvider(provider: FactoryProvider): void {
        ...
        const deps: Dependency[] = (provider.deps || []).map(
            (dep: Injectable) => ({ id: Store.providerId(dep) })
        );
