...
// --- Clase PdfReport ---
export class PdfReport implements ReportInterface {
    private htmlReport: ReportInterface;
    private html?: string;
    private options: Options;
    private binPath?: string;

    constructor(htmlReport: ReportInterface) {
    ...
    /**
     * Genera el PDF a partir del documento.
     */
    async render(
        document: DocumentInterface,
        parameters: Record<string, any> = {}
    ): Promise<string | null> {
        // Generar HTML con el report original
        this.html = await this.htmlReport.render(document, parameters);
        // Crear PDF
        return this.buildPdf(this.html);
    }

    /**
     * Define las opciones para wkhtmltopdf.
     */
    ...
    /**
     * Retorna el binario de wkhtmltopdf configurado.
     */
    getExporter(): typeof wkhtmltopdf {
        return wkhtmltopdf;
    }

    /**
     * Define la ruta del ejecutable wkhtmltopdf.
     */
    ...
    /**
     * Convierte HTML en PDF y devuelve el resultado como base64.
     */
    private buildPdf(html: string): Promise<string | null> {
        return new Promise((resolve, reject) => {
            const output = new PassThrough();
            const chunks: Uint8Array[] = [];

            // Inicia la conversión de HTML a PDF
            const pdfStream = wkhtmltopdf(html, options, (err) => {
